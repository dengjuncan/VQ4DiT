...
def compute_model_nbits(model: torch.nn.Module) -> int:
    ...
    compressed_dict = _compress_state_dict(model.state_dict(), float16_codebooks=False)

    total_size_bits = 0
    total_size_bits_codebook = 0
    total_size_bits_codes = 0
    ...
    for module_name, module in compressed_dict.items():
        ...
        if "codebook" in module_name:
            total_size_bits_codebook += module_size_bits
        ...
        if "codebook" in module_name:
            ...
            # print(f"{codes_param} has {n_codes} codes and takes up {module_size_bits / 8} bytes")

            total_size_bits += module_size_bits

            total_size_bits_codes += module_size_bits

    return total_size_bits, total_size_bits_codebook, total_size_bits_codes
