...
def reshape_weightlike_cin(weight,d):
    """
    conv: C_out x C_in x k x k -> (C_out x C_in /d x k x k) x d
    fc: C_out x C_in -> (C_out x C_in /d) x d
    """
    if len(weight.size()) == 4:
        c_out, c_in, k, k = weight.size()
        unroll1=torch.cat(weight.chunk(c_in//d,dim=1),dim=0)
        unroll2=torch.cat(unroll1.chunk(k,dim=2),dim=0)
        unroll3=torch.cat(unroll2.chunk(k,dim=3),dim=0).squeeze()

        return unroll3
    else:
        c_out, c_in = weight.size()
        fc_unroll=torch.cat(weight.chunk(c_in//d,dim=1),dim=0)
        return fc_unroll

def reshape_back_weight_cin(weight, k=3, conv=True, d=16, c_out=64, c_in=64):
    """
    (C_out x C_in /d x k x k) x d -> C_out x C_in x k x k.
    """

    if conv:
        #shape_back=torch.zeros(c_out,c_in,k,k).cuda()
        #print("shape_back_device:{}".format(shape_back.device))
        #for i in range(0,k*k):
        #    for m in range(0,c_in//d):
        #            for j in range(0,c_out):
        #                shape_back[j,m*d:m*d+d,i%k,i//k]=weight[i*c_in//d*c_out+m*c_out+j,:]
        roll_back=weight.unsqueeze(2).unsqueeze(3)
        roll_back=torch.cat(roll_back.chunk(k,dim=0),dim=3)
        roll_back=torch.cat(roll_back.chunk(k,dim=0),dim=2)
        roll_back=torch.cat(roll_back.chunk(c_in//d,dim=0),dim=1)
        return roll_back
    else:
...
def reshape_weightlike_cout(weight,d):
    """
    conv: C_out x C_in x k x k -> (C_out /d x C_in x k x k) x d
    fc: C_out x C_in -> (C_out x C_in /d) x d
    """
    if len(weight.size()) == 4:
        c_out, c_in, k, k = weight.size()
        unroll1=torch.cat(weight.chunk(c_out//d,dim=0),dim=1)
        unroll2=torch.cat(unroll1.chunk(k,dim=2),dim=1)
        unroll3=torch.cat(unroll2.chunk(k,dim=3),dim=1).squeeze().transpose(0,1)

        return unroll3
    else:
        c_out, c_in = weight.size()
        fc_unroll=torch.cat(weight.chunk(c_out//d,dim=0),dim=1).t()
        return fc_unroll
    
def reshape_back_weight_cout(weight, k=3, conv=True, d=16, c_out=64, c_in=64):
    """
    (C_out /d x C_in x k x k) x d -> C_out x C_in x k x k.
    """
    ...
    if conv:
        ...
        #            for j in range(0,c_out):
        #                shape_back[j,m*d:m*d+d,i%k,i//k]=weight[i*c_in//d*c_out+m*c_out+j,:]
        roll_back=weight.transpose(0,1)
        roll_back=roll_back.unsqueeze(2).unsqueeze(3)
        roll_back=torch.cat(roll_back.chunk(k,dim=1),dim=3)
        roll_back=torch.cat(roll_back.chunk(k,dim=1),dim=2)
        roll_back=torch.cat(roll_back.chunk(c_out//d,dim=1),dim=0)
        return roll_back
    else:
        fc_rollback = weight.transpose(0,1)
        fc_rollback=torch.cat(fc_rollback.chunk(c_out//d,dim=1),dim=0)
        return fc_rollback
...
def reshape_back_hwpe(unroll, h, w, cin, cout, blockh, blockw):
  ...
  '''
  new_tensor = torch.zeros(cout,cin,h,w).to('cuda:0')
  index = 0
  for i in range(h):
    for j in range(w):
...
def reshape_hwpe(weight, h, w, cin, cout, blockh, blockw):
  ...
  '''
  new_tensor = torch.zeros(blockw , h*w*cin*cout//blockh).to('cuda:0')
  index = 0
  for i in range(h):
    for j in range(w):
      for l in range(math.ceil(cin/blockh)):
        for m in range(cout//blockw):
          for n in range(cin%blockh):
            new_tensor[:,index] = weight[m*blockw:m*blockw+blockw, l*cin//blockh+n, i,j].squeeze()
            index+=1
  return new_tensor

def reshape_weightlike(weight, direction, d):
    if direction == 'input':
        reshape_weight = reshape_weightlike_cin(weight, d)
    elif direction == 'output':
        reshape_weight = reshape_weightlike_cout(weight, d)

    return reshape_weight

def reshape_back_weight(direction, M_hat_unrolled, k, c_out, c_in, d, isconv):
    if direction == 'output':
        M_hat_reshaped = reshape_back_weight_cout(M_hat_unrolled, k=k, conv=isconv, c_out=c_out, c_in=c_in, d=d)
    elif direction == 'input':
        M_hat_reshaped = reshape_back_weight_cin(M_hat_unrolled, k=k, conv=isconv, c_out=c_out, c_in=c_in, d=d)
