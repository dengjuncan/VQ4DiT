...
class Attention(nn.Module):
    ...
    def split_qkv(self):
        self.split_qkv_flag = True
        dim = self.qkv.in_features
        device = self.qkv.weight.device
        dtype = self.qkv.weight.dtype
        self.q = nn.Linear(dim, dim, bias=self.qkv.bias is not None).to(device=device, dtype=dtype)
        self.k = nn.Linear(dim, dim, bias=self.qkv.bias is not None).to(device=device, dtype=dtype)
        self.v = nn.Linear(dim, dim, bias=self.qkv.bias is not None).to(device=device, dtype=dtype)
        self.q.weight.data = self.qkv.weight.data[:dim, :].clone()
        self.k.weight.data = self.qkv.weight.data[dim:2*dim, :].clone()
        self.v.weight.data = self.qkv.weight.data[2*dim:, :].clone()
        if self.qkv.bias is not None:
            self.q.bias.data = self.qkv.bias.data[:dim].clone()
            self.k.bias.data = self.qkv.bias.data[dim:2*dim].clone()
            self.v.bias.data = self.qkv.bias.data[2*dim:].clone()
        del self.qkv

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, N, C = x.shape
        if self.split_qkv_flag:
