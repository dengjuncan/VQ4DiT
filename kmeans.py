...
def weighted_distance(x, centroids):
    # x: [n_samples, n_features]
    # centroids: [n_clusters, n_features]
    diff = x.unsqueeze(1) - centroids.unsqueeze(0)  # [n_samples, n_clusters, n_features]
    weighted_sq_dist = (diff ** 2) 
    return torch.sqrt(weighted_sq_dist.sum(dim=-1))  # [n_samples, n_clusters]


def kmeans_plusplus_init_simple(X, num_clusters):
    """
    ...
    # Calculate the minimum distance from each point to the selected center
    distances = torch.full((n,), float('inf'), device=device)
    
    for c in range(1, num_clusters):
        # Calculate the distance from all current points to the latest center
        latest_centroid = centroids[c-1:c]
        current_distances = weighted_distance(X, latest_centroid).squeeze(1)
        
        # Update minimum distance
        distances = torch.minimum(distances, current_distances)
        
        # Select the next center based on the probability of the square of the distance
...
def kmeans(
    ...
    if codebook is None:
        codebook = kmeans_plusplus_init_simple(training_set, k)
    else:
